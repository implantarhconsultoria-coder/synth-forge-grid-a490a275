...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    supabase.from('fechamentos_filial').select('status, filial_id').eq('competencia', comp).then(({ data }) => {
      ...
      const totalFiliaisComFechamento = new Set(arr.map(f => f.filial_id)).size;
      const pendentes = Math.max(0, companies.length - totalFiliaisComFechamento);
