...
export function MobileBottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 px-2 pb-2 pt-2">
      <div className="glass rounded-2xl border border-border/60 backdrop-blur-xl bg-background/85 grid grid-cols-4 gap-1 px-1 py-1.5 shadow-[0_-8px_30px_rgba(56,189,248,0.15)]">
        {ITEMS.map((it) => {
          const Icon = it.icon;
          const active = pathname === it.to;
          if ("primary" in it && it.primary) {
            return (
              <Link key={it.to} to={it.to} className="flex flex-col items-center justify-center -mt-5">
